...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (strcmp(cmd, "create") == 0) {
    ...
    const MeshInterface *mIMesh = opcode->GetMeshInterface();

	int n_c = (int) mxGetN(prhs[ix_c1]);
	int ix_hit=0,ix_dist=1,ix_tridx=2,ix_bary=3,ix_isect=4;
    ...
    float *isectptr = 0;
    VertexPointers VP;
    IceMaths::Point v1,v2,v3;
    float b1,b2,b3;
    if (nlhs > ix_isect) {
        plhs[ix_isect] = mxCreateNumericMatrix(3, n_c, mxSINGLE_CLASS, mxREAL);
        isectptr = (float *) mxGetData(plhs[ix_isect]);
    }
	...
	for (i=0; i<n_c; i++) {
      ...
      if (nlhs > ix_isect) {
          if (hit) {
		      b1 = (1-baryptr[2*i]-baryptr[2*i+1]);
              b2 = baryptr[2*i];
              b3 = baryptr[2*i+1];
              mIMesh->GetTriangle(VP, tridxptr[i]-1);
              v1 = *VP.Vertex[0];
              v2 = *VP.Vertex[1];
              v3 = *VP.Vertex[2];
              isectptr[3*i] = b1*v1.x + b2*v2.x + b3*v3.x;
              isectptr[3*i+1] = b1*v1.y + b2*v2.y + b3*v3.y;
              isectptr[3*i+2] = b1*v1.z + b2*v2.z + b3*v3.z;
          }
		  else {
			  isectptr[3*i] = nan;
			  isectptr[3*i+1] = nan;
	...
	IceMaths::Point* vc = (IceMaths::Point*)mIMesh->GetVerts();
